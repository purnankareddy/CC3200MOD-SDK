...
int enddevicestatus=0;
int philipsidstatus=0;
	...
	int udppackets=0,a=0,b=0;
	 ...
	 // puts(message);
		...
		if(a==0 || b<=10)
		{
			...
			if(b<10)
			{
				if(strstr(msgbuf,"NOTIFY") && strstr(msgbuf,"Belkin"))
				{
					int find=0;
...
void insertFirst(char key[2], char data[400]) {
   //create a link
   struct node *link = (struct node*) malloc(sizeof(struct node));
	
   //link->bulbid = key;
   //link->status = data;
     strcpy(link->bulbid,key);
     strcpy(link->status,data);
	...
	//IOT_INFO("parser------%s-------\n",&parser);
		...
		if(philipsget==1)
		{
			...
			for(i=1;i<noOfToken;i+=42)
			{
				...
				//strncpy(vinaybulbid,keyString,strlen(keyString));
				//IOT_INFO("Key: %s\n", vinaybulbid);
				IOT_INFO("Key: %s\n", p_bulbs.bid);
				...
				IOT_INFO("Key: %s\n", p_bulbs.statuses);
				//strncpy(vinaystatus,keyString,strlen(keyString));
				//IOT_INFO("Key: %s\n", vinaystatus);
				IOT_INFO("Key: %s\n", keyString);
				//}
//				IOT_INFO("Key: %s\n", keyString);
//				IOT_INFO("my Key: %s\n", keyString[1]);
				insertFirst(vinaybulbid,vinaystatus);
        ...
        uint16_t topicNameLen, IoT_Publish_Message_Params *params, void *pData)
{
    ...
    int size = (int)params->payloadLen;
    IOT_INFO("--------%d------\n",size);
    ...
    strcpy(string,(char *)params->payload);
...
*/
    ...
    if(!strcmp(word[2],"on"))
    lRetVal = HTTPPostMethodon(&httpClient);
...
ch=3;
 ...
 if(!strncmp((char *)params->payload ,"discover", 8)){
	IOT_INFO("----Discover-----");
	//sri_recev();	
	//discover();
	//vinayudp();
 ...
 if(!strncmp((char *)params->payload ,"multicast", 9)){
...
recievepack();
 ...
 if(!strncmp((char *)params->payload ,"demouser", 8)){
	IOT_INFO("----SENDRevice-----");
//recievepack();
}

 if(!strncmp((char *)params->payload ,"philps get", 10)){
philipsget=1;
//enddevicestatus=1;
...
IOT_INFO("HTTP Get Begin:\n\r");
    ...
    if(!strncmp((char *)params->payload ,"philps off", 10)){
IOT_INFO("philips Function Call...\n");
//getip();
strcpy(HOST_NAME,philips_uri);
HOST_PORT = 80;
...
*/
 ...
 IOT_INFO("HTTP Put Begin:\n\r");
    ...
    if(!strncmp((char *)params->payload ,"philps on", 9)){
IOT_INFO("philips Function Call...\n");
//getip();
strcpy(HOST_NAME,philips_uri);
HOST_PORT = 80;
...
*/
 ...
 IOT_INFO("HTTP Put Begin:\n\r");
    ...
    lRetVal = HTTPPutMethodon(&httpClient);
    if(lRetVal < 0)
    {
    	IOT_INFO("HTTP Put failed.\n\r");
    }
    ...
    if(!strncmp((char *)params->payload ,"philps config", 13)){
...
*/
    IOT_INFO("\n\r");
     IOT_INFO("HTTP Post Begin:\n\r");
    lRetVal = philipsconfig(&httpClient);
    if(lRetVal < 0)
    {
    	IOT_INFO("HTTP Post failed.\n\r");
    }
    ...
    if(!strncmp((char *)params->payload ,"belkin on", 9)){
IOT_INFO("Belkin Function Call...\n");
long lRetVal = -1;
...
if(!strncmp((char *)params->payload ,"belkin off", 10)){
IOT_INFO("Belkin Function Call...\n");
long lRetVal = -1;
strcpy(HOST_NAME,belkin_uri[0]);
HOST_PORT = 49153;
...
if (!strncmp((char *)params->payload , "gpio all on", 11)){
    IOT_INFO("\n GPIO OFF using COPY..  \n");
    GPIOPinWrite(GPIOA3_BASE, 0x10, 0x10);
    GPIOPinWrite(GPIOA1_BASE, 0x80, 0x80);
    GPIOPinWrite(GPIOA1_BASE, 0x40, 0x40);
...
if (!strncmp((char *)params->payload , "gpio all off", 12)){
    IOT_INFO("\n GPIO OFF using COPY..  \n");
...
IOT_INFO("\n....RUNNING....AWS_IOT...\n");
	...
	sprintf(cPayload, "{ \n \"socketId\" : \"%d\",\n\"status\" : \"%d\",\n\"power\" : \"%f\",\n\"dexterpi_id\" : \"DXTRPIE1\",\n\"userId\" : \"%s\"\n}",ch,status,power(ch),userid);
	...
	if(enddevicestatus==1)
	{
		...
		char *pubtopicNames = "theja";//"enddevices/status";
    		int pubtopicNamesLen = strlen(pubtopicNames);
		
		if(philipsidstatus==0)
		{
			sprintf(cPayload, "{ \n \"PhilipsIPStatus\" : [\"%s\",\"not yet get\"],\n\"Belkin1IPStatus\" : \"%s\",\n\"Belkin2IPStatus\" : \"%s\",\n\"Belkin3IPStatus\" : \"%s\",\n\"Belkin4IPStatus\" : \"%s\",\n\"Belkin5IPStatus\" : \"%s\",\n\"Belkin6IPStatus\" : \"%s\",\n\"Belkin7IPStatus\" : \"%s\",\n\"Belkin8IPStatus\" : \"%s\",\n\"Belkin9IPStatus\" : \"%s\",\n\"Belkin10IPStatus\" : \"%s\",\n\"dexterpi_id\" : \"DEXT01\"\n}",philips_uri,belkin_uri[0],belkin_uri[1],belkin_uri[2],belkin_uri[3],belkin_uri[4],belkin_uri[5],belkin_uri[6],belkin_uri[7],belkin_uri[8],belkin_uri[9]);
		}
		else if(philipsidstatus==1)
		{
			sprintf(cPayload, "{ \n \"PhilipsIPStatus\" : [\"%s\",\"please press link button on hue bridge\"],\n\"Belkin1IPStatus\" : \"%s\",\n\"Belkin2IPStatus\" : \"%s\",\n\"Belkin3IPStatus\" : \"%s\",\n\"Belkin4IPStatus\" : \"%s\",\n\"Belkin5IPStatus\" : \"%s\",\n\"Belkin6IPStatus\" : \"%s\",\n\"Belkin7IPStatus\" : \"%s\",\n\"Belkin8IPStatus\" : \"%s\",\n\"Belkin9IPStatus\" : \"%s\",\n\"Belkin10IPStatus\" : \"%s\",\n\"dexterpi_id\" : \"DEXT01\"\n}",philips_uri,belkin_uri[0],belkin_uri[1],belkin_uri[2],belkin_uri[3],belkin_uri[4],belkin_uri[5],belkin_uri[6],belkin_uri[7],belkin_uri[8],belkin_uri[9]);			
//sprintf(cPayload, "{ \n \"PhilipsIPStatus\" : \"%d\",\n\"BelkinIPStatus\" : \"%d\",\n\"PhilipsUserIDStatus\" : \"please press link button on hue bridge\",\n\"dexterpi_id\" : \"DEXT01\"\n}",philipsipstatus,belkinipstatus);
		}
		else if(philipsidstatus==2)
		{
			sprintf(cPayload, "{ \n \"PhilipsIPStatus\" : [\"%s\",\"have an new userid\"],\n\"Belkin1IPStatus\" : \"%s\",\n\"Belkin2IPStatus\" : \"%s\",\n\"Belkin3IPStatus\" : \"%s\",\n\"Belkin4IPStatus\" : \"%s\",\n\"Belkin5IPStatus\" : \"%s\",\n\"Belkin6IPStatus\" : \"%s\",\n\"Belkin7IPStatus\" : \"%s\",\n\"Belkin8IPStatus\" : \"%s\",\n\"Belkin9IPStatus\" : \"%s\",\n\"Belkin10IPStatus\" : \"%s\",\n\"dexterpi_id\" : \"DEXT01\"\n}",philips_uri,belkin_uri[0],belkin_uri[1],belkin_uri[2],belkin_uri[3],belkin_uri[4],belkin_uri[5],belkin_uri[6],belkin_uri[7],belkin_uri[8],belkin_uri[9]);
...
/////////added for philips get///////
		if(philipsget==1)
		{
			...
			ptr = head;
   			...
   			while(ptr != NULL) {
      			IOT_INFO("(%s,%s) ",ptr->bulbid,ptr->status);
			sprintf(cPayload,"{\n\"bulbid\" : \"%s\",\n\"status\" : \"%s\"\n}",ptr->bulbid,ptr->status);
			IOT_INFO("cpayload---%s----",cPayload);
        		/* Recalculate string len to avoid truncation in subscribe callback */
        		paramsQOS1.payloadLen = strlen(cPayload);
        		do {
